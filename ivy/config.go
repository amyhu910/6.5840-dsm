...
func (cfg *config) makeClient() *Client {
	...
	ck := MakeClient(end, SERVERID)
	cfg.clients[ck] = endname
	cfg.nextClientId++
...
func (cfg *config) ConnectClientUnlocked(ck *Client) {
	//log.Printf("ConnectClient %v\n", ck)
	endname := cfg.clients[ck]
	cfg.net.Enable(endname, true)
}

func (cfg *config) ConnectClient(ck *Client) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.ConnectClientUnlocked(ck)
}
...
func (cfg *config) StartServer() {
	cfg.central = MakeCentral()

	kvsvc := labrpc.MakeService(cfg.central)
